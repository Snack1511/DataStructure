...
int RemoveEdge(Graph* pGraph, int EnterVert, int OutVert){
	...
	if (pGraph != NULL) {
		Node.data.vertID = OutVert;
		removeLLElement(pGraph->ppAdjacentList[EnterVert], GetPosition(pGraph->ppAdjacentList[EnterVert], Node));
		if (pGraph->DirectMode == UNDIRECT) {
			Node.data.vertID = EnterVert;
			removeLLElement(pGraph->ppAdjacentList[OutVert], GetPosition(pGraph->ppAdjacentList[OutVert], Node));
		}
	}
	return ret;
}
